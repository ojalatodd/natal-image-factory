...
**Current state:** Phase 2 in progress. Core still image pipeline (Phase 1) complete with all six stages implemented. Phase 2 adds: source adapter management UI (enable/disable, priority), project re-generation, DALL-E 3 AI image fallback when no public-domain candidates found, and two additional source adapters (The Met, Smithsonian Open Access). Five source adapters now registered total. All AI stages gracefully degrade when OPENAI_API_KEY is not set. Video processing (trim/normalize, Ken Burns) is deferred to Phase 3.
